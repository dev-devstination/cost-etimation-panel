...
export async function updateSubcategoryAction(
  ...
  const { id, ...body } = data
  try {
    await fetcher(`/resources/subcategories/${id}`, {
      method: "PATCH",
      body,
    })

    logger.info("Subcategory updated successfully", { data })
    revalidatePath("/resources/subcategories")
...
export async function addSubcategoryAction(
  ...
  try {
    await fetcher("/resources/subcategories", {
      method: "POST",
      body: data,
    })
...
export async function subcategoryStateAction(
  ...
  try {
    if (data.state) {
      await fetcher(`/resources/subcategories/${data.id}/activate`, {
        method: "PATCH",
      })
    ...
    await fetcher(`/resources/subcategories/${data.id}/deactivate`, {
      method: "PATCH",
    })
