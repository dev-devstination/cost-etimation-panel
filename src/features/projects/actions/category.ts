...
import { CategoryFormData } from "@/features/projects/schemas/category"
...
export async function updateCategoryAction(
  ...
  const { id, ...body } = data
  try {
    await fetcher(`/projects/categories/${id}`, {
      method: "PATCH",
      body,
    })

    logger.info("Category updated successfully", { data })
    revalidatePath("/projects/categories")
...
export async function addCategoryAction(
  ...
  try {
    await fetcher("/projects/categories", {
      method: "POST",
      body: data,
    })
...
export async function categoryStateAction(
  ...
  try {
    if (data.state) {
      await fetcher(`/projects/categories/${data.id}/activate`, {
        method: "PATCH",
      })
    ...
    await fetcher(`/projects/categories/${data.id}/deactivate`, {
      method: "PATCH",
    })
